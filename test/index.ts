...
describe(CollectionConfig.contractName, function () {
  ...
  it('Before any sale', async function () {
    ...
    await expect(contract.connect(whitelistedUser).mintLizard(whitelistedUser.address, 1, {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The contract is paused!');
    await expect(contract.connect(whitelistedUser).whitelistMintLizard(whitelistedUser.address, 1, [], {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The whitelist sale is not enabled!');
    await expect(contract.connect(holder).mintLizard(holder.address, 1, {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The contract is paused!');
    await expect(contract.connect(holder).whitelistMintLizard(holder.address, 1, [], {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The whitelist sale is not enabled!');
  ...
  it('Whitelist sale', async function () {
    ...
    await contract.connect(whitelistedUser).whitelistMintLizard(
    ...
    await expect(contract.connect(holder).whitelistMintLizard(
			holder.address,
      1,
      merkleTree.getHexProof(keccak256(await whitelistedUser.getAddress())),
      {value: getPrice(SaleType.WHITELIST, 1)},
  ...
  it('Supply checks (long)', async function () {
    ...
    await Promise.all([...Array(iterations).keys()].map(async () => await contract.connect(whitelistedUser).mintLizard(whitelistedUser.address, maxMintAmountPerTx, {value: getPrice(SaleType.PUBLIC_SALE, maxMintAmountPerTx)})));

    // Try to mint over max supply (before sold-out)
    await expect(contract.connect(holder).mintLizard(holder.address, lastMintAmount + 1, {value: getPrice(SaleType.PUBLIC_SALE, lastMintAmount + 1)})).to.be.revertedWith('Max supply exceeded!');
    await expect(contract.connect(holder).mintLizard(holder.address, lastMintAmount + 2, {value: getPrice(SaleType.PUBLIC_SALE, lastMintAmount + 2)})).to.be.revertedWith('Max supply exceeded!');

    expect(await contract.totalSupply()).to.equal(expectedTotalSupply);

    // Mint last tokens with owner address and test walletOfOwner(...)
    await contract.connect(owner).mintLizard(owner.address, lastMintAmount, {value: getPrice(SaleType.PUBLIC_SALE, lastMintAmount)});
    ...
    await expect(contract.connect(whitelistedUser).mintLizard(whitelistedUser.address, 1, {value: getPrice(SaleType.PUBLIC_SALE, 1)})).to.be.revertedWith('Max supply exceeded!');
